...
/**
 * https://github.com/Tencent/QMUI_Android/blob/1.x/qmui/src/main/java/com/qmuiteam/qmui/widget/QMUIRadiusImageView.java
...
public class QMUIRadiusImageView extends AppCompatImageView {
  ...
  public QMUIRadiusImageView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    mBorderWidth =
        array.getDimensionPixelSize(R.styleable.QMUIRadiusImageView_qmui_border_width, 0);
    mBorderColor =
        array.getColor(R.styleable.QMUIRadiusImageView_qmui_border_color, DEFAULT_BORDER_COLOR);
  ...
  public void setSelectedMaskColor(@ColorInt int selectedMaskColor) {
    if (mSelectedMaskColor != selectedMaskColor) {
      ...
      if (mSelectedMaskColor != Color.TRANSPARENT) {
        mSelectedColorFilter =
            new PorterDuffColorFilter(mSelectedMaskColor, PorterDuff.Mode.DARKEN);
      } else {
  ...
  protected void updateScaleTypeMatrix(@NonNull Matrix matrix, @NonNull Bitmap bitmap,
      RectF drawRect) {
  ...
  private void drawBitmap(Canvas canvas, int borderWidth) {
    ...
    if (mIsCircle) {
      canvas.drawCircle(mRectF.centerX(), mRectF.centerY(),
          (Math.min(mRectF.width() / 2, mRectF.height() / 2)) - halfBorderWidth, mBitmapPaint);
    } else {
