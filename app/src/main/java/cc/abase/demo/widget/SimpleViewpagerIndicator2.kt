...
/**
 * description: 文字大小变化效果
 * Author:Khaos116
 * Date:2020/8/12
 * Time:20:41
 */
class SimpleViewpagerIndicator2 @JvmOverloads constructor(
    con: Context,
  ...
  private var parentPadding = 0 //整个Tab的内边距
  ...
  fun setViewPager(viewPager: ViewPager): SimpleViewpagerIndicator2 {
  ...
  fun setOnPageChangeListener(listener: OnPageChangeListener?): SimpleViewpagerIndicator2 {
  ...
  private fun init() {
    ...
    parentPadding = dp2px(parentPadding.toFloatMy())
    ...
    tabsContainer.setPadding(parentPadding, 0, parentPadding, 0)
  ...
  private fun addTab(position: Int, title: String) {
    ...
    tab.setSingleLine()
    tab.text = title
    tab.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabTextSize.toFloatMy())
    val widthText = tab.paint.measureText(title)
    if (tabBackgroundResId != 0) tab.setBackgroundResource(tabBackgroundResId)
    val wrapTabLayoutParams = LinearLayout.LayoutParams(-2, -1) //宽度包裹内容
    if (expand) {
      tab.setPadding(tabPadding, 0, tabPadding, 0)
    } else {
      wrapTabLayoutParams.width = (widthText + tabPadding * 2f).toInt() + 1
    }
  ...
  private fun scrollToChild(position: Int, offset: Int) {
    ...
    var newScrollX = tabsContainer.getChildAt(position).left + offset - parentPadding
  ...
  fun setExpand(expand: Boolean): SimpleViewpagerIndicator2 {
    this.expand = expand
    return this
  }

  fun setNoClickPosition(position: Int, call: (() -> Unit)? = null): SimpleViewpagerIndicator2 {
    mNoClickPosition = position
    mNoClickCall = call
    return this
  }

  fun setIndicatorWrapText(indicatorWrapText: Boolean): SimpleViewpagerIndicator2 {
    this.indicatorWrapText = indicatorWrapText
    return this
  }

  //setter--------------------------------------------------------------------------------------------
  fun setTabWidth(tabWidth: Int, tabRound: Int): SimpleViewpagerIndicator2 {
    this.tabWidth = tabWidth
    this.tabRound = tabRound
    return this
  }

  fun setIndicatorColor(indicatorColor: Int): SimpleViewpagerIndicator2 {
    this.indicatorColor = indicatorColor
    return this
  }

  fun setIndicatorHeight(indicatorHeight: Int): SimpleViewpagerIndicator2 {
  ...
  fun setTabTextSize(tabTextSize: Int): SimpleViewpagerIndicator2 {
    this.tabTextSize = tabTextSize
    return this
  }

  fun setTabTextColor(tabTextColor: Int): SimpleViewpagerIndicator2 {
    this.tabTextColor = tabTextColor
    return this
  }

  fun setTabTypeface(tabTypeface: Typeface?): SimpleViewpagerIndicator2 {
    this.tabTypeface = tabTypeface
    return this
  }

  fun setTabTypefaceStyle(tabTypefaceStyle: Int): SimpleViewpagerIndicator2 {
    this.tabTypefaceStyle = tabTypefaceStyle
    return this
  }

  fun setTabBackgroundResId(tabBackgroundResId: Int): SimpleViewpagerIndicator2 {
    this.tabBackgroundResId = tabBackgroundResId
    return this
  }

  fun setTabPadding(tabPadding: Int): SimpleViewpagerIndicator2 {
    this.tabPadding = tabPadding
    return this
  }

  fun setParentPadding(tabPadding: Int): SimpleViewpagerIndicator2 {
    this.parentPadding = tabPadding
    return this
  }

  fun setSelectedTabTextSize(selectedTabTextSize: Int): SimpleViewpagerIndicator2 {
    this.selectedTabTextSize = selectedTabTextSize
    return this
  }

  fun setSelectedTabTextColor(selectedTabTextColor: Int): SimpleViewpagerIndicator2 {
    this.selectedTabTextColor = selectedTabTextColor
    return this
  }

  fun setSelectedTabTypeface(selectedTabTypeface: Typeface?): SimpleViewpagerIndicator2 {
    this.selectedTabTypeface = selectedTabTypeface
    return this
  }

  fun setSelectedTabTypefaceStyle(selectedTabTypefaceStyle: Int): SimpleViewpagerIndicator2 {
    this.selectedTabTypefaceStyle = selectedTabTypefaceStyle
    return this
  }

  fun setScrollOffset(scrollOffset: Int): SimpleViewpagerIndicator2 {
    this.scrollOffset = scrollOffset
    return this
  }

  fun setTabTransY(tansYdp: Float): SimpleViewpagerIndicator2 {
    tabTransY = tansYdp
    return this
  }

  fun setTextTransY(tansYdp: Float): SimpleViewpagerIndicator2 {
    textTransY = tansYdp
    return this
  }
    ...
    override fun onPageScrolled(position: Int, positionOffset: Float, positionOffsetPixels: Int) {
      ...
      for (i in 0 until count) {
        ...
        if (i != sel) {
          ...
          tv.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabTextSize.toFloatMy())
