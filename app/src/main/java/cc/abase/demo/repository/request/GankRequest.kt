...
import cc.abase.demo.repository.parse.GankParameterizedTypeImpl
...
internal class GankRequest private constructor() : BaseRequest() {
  ...
  //直接请求数据，不走缓存-->结果非列表
  internal inline fun <reified T> startRequest(
    request: Request
  ...
  //直接请求数据，不走缓存-->结果列表
  internal inline fun <reified T> startRequestList(
    request: Request
  ): Single<GankResponse<MutableList<T>>> {
    return request.rxString()
  ...
  private inline fun <reified T> flatMapSingle(
    ...
    return if (result.component2() == null) {
      Single.just(
          converGankData(result.component1(), GankParameterizedTypeImpl.typeOne(T::class.java))
      )
    } else {
  ...
  private inline fun <reified T> flatMapSingleList(
    ...
    return if (result.component2() == null) {
      Single.just(
          converGankData(result.component1(), GankParameterizedTypeImpl.typeList(T::class.java))
      )
    } else {
      Single.error(converFuelError(result.component2()))
    }
