...
object CoilDownImgUtils {
    ...
    fun downloadImgByCoil(url: String?, call: (start: Boolean, end: Boolean, sucPath: String, msg: String) -> Unit) {
        if (url.isNullOrBlank()) { //下载地址不存在
            call.invoke(false, true, "", R.string.下载地址不存在.xmlToString())
            return
        }
        ...
        if (destDir.isNullOrBlank()) { //没有获取到DCIM和Pictures文件夹
            call.invoke(false, true, "", R.string.相册不能访问.xmlToString())
            return
        }
        launchError(handler = { _, e -> call.invoke(false, true, "", e.message ?: R.string.下载出错请重试.xmlToString()) }) {
            call.invoke(true, false, "", R.string.开始下载.xmlToString())
            withContext(Dispatchers.IO) { delay(500) }.let { //延迟500ms，方便显示loading
                ...
                if (url.startsWith("http")) { //是http开头
                        ...
                        onSuccess = { _, _ ->
                            ...
                            if (cacheFile?.exists() == true) {
                                ...
                                if (result) { //发送广播刷新图片
                                    ...
                                    call.invoke(false, true, destF.path, R.string.下载成功.xmlToString())
                                } else {
                                    call.invoke(false, false, "", R.string.下载成功插入相册失败.xmlToString())
                                }
                            } else {
                                call.invoke(false, true, "", R.string.下载成功未找到缓存文件.xmlToString())
                            }
                        }
                    ).build())
                } else { //本地文件
                    ...
                    if (cacheFile?.exists() == true) {
                        ...
                        if (result) {
                            Utils.getApp().sendBroadcast(Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, UriUtils.file2Uri(destF)))
                            call.invoke(false, true, destF.path, R.string.本地文件插入相册成功.xmlToString())
                        } else {
                            call.invoke(false, true, "", R.string.本地文件插入相册失败.xmlToString())
                        }
                    } else {
                        call.invoke(false, true, "", R.string.本地文件未找到.xmlToString())
                    }
                }
            }
        }
