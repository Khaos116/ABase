...
class LoginActivity : CommBindActivity<ActivityLoginBinding>() {
    ...
    private fun checkSubmit() {
        ...
        if (textAcc.isEmpty()) {
            viewBinding.loginInputAccount.hint = StringUtils.getString(R.string.请输入用户名)
        } else if (textAcc.isNotEmpty() && textAcc.length < LengthConstants.MIN_LEN_ACC) {
            viewBinding.loginInputAccount.hint = StringUtils.getString(R.string.用户名太短)
        } else {
            viewBinding.loginInputAccount.hint = ""
        }
        if (textPass.isEmpty()) {
            viewBinding.loginInputPassword.hint = StringUtils.getString(R.string.请输入密码)
        } else if (textPass.isNotEmpty() && textPass.length < LengthConstants.MIN_LEN_PASS) {
            viewBinding.loginInputPassword.hint = StringUtils.getString(R.string.密码太短)
        } else {
        ...
        if (result == BiometricInterface.HW_AVAILABLE) {
            ...
            mmkv?.decodeString("Biometric")?.let { author ->
                ...
                commAlertDialog(supportFragmentManager, cancelable = false, outside = false) {
                    ...
                    content = R.string.是否使用指纹登录.xmlToString()
                    confirmText = R.string.指纹登录.xmlToString()
                    cancelText = R.string.取消.xmlToString()
        ...
        provider.tryAuthenticate(this@LoginActivity, { cipher ->
            ...
            if (AppUtils.isAppForeground()) {
                ...
                if (code == BiometricInterface.ERROR_FAILED && failCount <= 5) {
                    R.string.验证失败请重试.xmlToast()
                    authenticate(provider, author)
                } else if (failCount > 5) {
                    R.string.失败次数太多暂停使用.xmlToast()
    ...
    private fun saveBiometric(account: String, password: String) {
        ...
        if (result == BiometricInterface.HW_AVAILABLE) {
            commAlertDialog(supportFragmentManager, cancelable = false, outside = false) {
                title = R.string.温馨提示.xmlToString()
                content = R.string.检测到您的手机支持指纹识别是否开启指纹登录.xmlToString()
                confirmText = R.string.开启.xmlToString()
                cancelText = R.string.忽略.xmlToString()
                cancelCallback = {
                    MainActivity.startActivity(mContext)
                }
                confirmCallback = {
