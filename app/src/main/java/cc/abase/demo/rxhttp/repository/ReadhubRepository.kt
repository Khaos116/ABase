...
object ReadhubRepository {
  //获得热门话题
  suspend fun getTopicList(lastOrder: Long = 0, readCache: Boolean = true): Await<MutableList<TopicBean>> {
    return RxHttp.get("${ReadhubUrl.Home.TOPIC}?pageSize=20${if (lastOrder <= 0) "" else "&lastCursor=$lastOrder"}")
      ...
      .setCacheMode(if (readCache && lastOrder == 0L) CacheMode.READ_CACHE_FAILED_REQUEST_NETWORK else CacheMode.ONLY_NETWORK) //先请求数据，失败再读取缓存
