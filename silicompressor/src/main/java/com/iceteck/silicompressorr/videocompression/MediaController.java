...
public class MediaController {
  ...
  public boolean convertVideo(final String sourcePath, File destDir, int outWidth, int outHeight, int outBitrate) {
    ...
    //=============================================修改代码1-Start===========================================//
    //用于计算压缩进度
    long originalDuration = Long.parseLong(retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION)) * 1000;
    //修改比特率
    ...
    //压缩后的文件前缀
    String filePrefix = EncryptUtils.encryptMD5File2String(originFile);
    //压缩完成后需要改为.mp4
    ...
    //压缩过程中显示.temp
    cacheFile = new File(destDir, filePrefix + ".temp");
    //可能存在上次压缩失败的文件，直接删除
    if (cacheFile.exists()) FileUtils.delete(cacheFile);
    //码率低于400不进行码率压缩
    if (originalBitrate < 400 * 1000) {
      bitrate = originalBitrate;
    } else if (originalBitrate > 2500 * 1000) {
      bitrate = 2500 * 1000;
    } else if (originalBitrate < 700 * 1000) {
      bitrate = 400 * 1000;
    } else {
      bitrate = (int) (originalBitrate * 0.7f);
    }
    //最大允许宽度为1080
    float scale = 1;
    if (originalWidth > 1080 && (rotationValue == 0 || rotationValue == 180)) {
      scale = 1080f / originalWidth;
    } else if (originalHeight > 1080 && (rotationValue == 90 || rotationValue == 270)) {
      scale = 1080f / originalHeight;
    }
    ...
    if (resultWidth != 0 && resultHeight != 0) {
      ...
      try {
        ...
        //=============================================修改代码3-Start===========================================//
        if (resultWidth != originalWidth || resultHeight != originalHeight || bitrate != originalBitrate) {
          //===========================================修改代码3-END=============================================//
          ...
          if (videoIndex >= 0) {
            ...
            try {
              ...
              if (Build.VERSION.SDK_INT < 18) {
                ...
                Log.e("tmessages",
                    "codec = " + codecInfo.getName() + " manufacturer = " + manufacturer + "device = " + Build.MODEL);
              } else {
                    ...
                    if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                      ...
                      if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                        ...
                        if (Build.VERSION.SDK_INT >= 18) {
                          ...
                          int inputBufIndex = encoder.dequeueInputBuffer(TIMEOUT_USEC);
                          if (inputBufIndex >= 0) {
                            encoder.queueInputBuffer(inputBufIndex, 0, 1, info.presentationTimeUs,
...
*/
    ...
    //=============================修改代码5-Start=============================//
    //重命名
    FileUtils.rename(cachedFile, resultFile.getName());
    cachedFile = resultFile;
    //=============================修改代码5-END==============================//
    return true;
  }
